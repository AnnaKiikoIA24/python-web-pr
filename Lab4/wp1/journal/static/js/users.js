...
const checkUser = async () => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      window.location.href = `../${!user.role ? "teacher" : "student"}.html`;
    // Помилки верифікації
    } else {
...
const getUsers = async (idGroup, callbackFn) => {

  try {
    // надсилаємо запит 
    const response = await fetch(`/api/users${idGroup ? "?idGroup=" + idGroup : ""}`, {
      method: "GET"
    });
    ...
    // Помилки 
      ...
      window.alert(`Помилка запиту даних про користувачів, статус: ${response.status}`);               
    }
  }
  catch(error) {
    console.log("getUsers:", error);    
    window.alert(`Помилка запиту даних про користувачів: ${error}`);  
  }
}
...
const addUser = async () => {
  ...
  try {
    ...
    if (response.ok) {
      const user = await response.json();
      window.location.href = `/static/pages/${!user.role ? "teacher" : "student"}.html`;
    // Помилки верифікації
    } else {
      // Аналізуємо статус відповіді
      switch (response.status) {
...
const navUserInfo = (user) => {
  document.getElementById("idUserName").innerHTML = 
    `Користувач: <small><strong>${user.firstName} ${user.lastName}</strong> (${user.role ? "Студент, Група " +  user.name : "Викладач"})</small>`;
